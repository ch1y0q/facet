...
> 🌐 [English](../DEPLOYMENT.md) · [Français](../fr/DEPLOYMENT.md) · [Deutsch](../de/DEPLOYMENT.md) · **Italiano** · [Español](../es/DEPLOYMENT.md) · [Português](../pt/DEPLOYMENT.md) · [简体中文](../zh/DEPLOYMENT.md)
...
### Dove va il cestino di un container

Quando `viewer.cull.allow_trash` è attivo, `trash_rejects` chiama `send2trash`, che implementa la specifica del cestino freedesktop.org e sceglie tra due destinazioni a seconda che il file e la tua `$HOME` condividano o meno lo stesso filesystem. Nella disposizione del container fornita — foto montate in bind su `/data/photos`, in esecuzione come uid 1000 con `HOME=/home/facet` — non accade mai: `/data/photos` è un proprio punto di montaggio all'interno del container (`os.path.ismount()` restituisce `True` per esso) e si trova su un device diverso rispetto alla `$HOME` overlay del container — confronta `os.stat(...).st_dev` per una foto e per `$HOME` se vuoi verificarlo sul tuo deployment; i valori stessi variano da host a host, conta solo il fatto che siano diversi. `send2trash` prende quindi il ramo *cestino di volume* della specifica, non quello del cestino della home: un file scartato finisce in `/data/photos/.Trash-1000/files/<nome>`, con un `<nome>.trashinfo` corrispondente che registra `Path=` e `DeletionDate=` sotto `/data/photos/.Trash-1000/info/`. Quella directory viene creata con modalità `0700`.

Poiché `/data/photos` è il bind mount stesso, `.Trash-1000` è visibile e intatta sul lato **host** del mount — un semplice `mv` fuori da `files/` ripristina una foto cestinata, senza alcun accesso al container. Lo scanner di Facet non ci inciamperà per caso: `os.walk` esclude le directory nascoste quando `scanning.skip_hidden_directories` è vero (impostazione predefinita), quindi un file cestinato resta invisibile a una nuova scansione con quel flag attivo, e viene ritrovato se lo disattivi.

Due casi si comportano diversamente:

- **Nessun bind mount per l'albero fotografico** (ad es. un `docker run` che memorizza le foto sul filesystem proprio del container) mette le foto sullo stesso device di `$HOME`, quindi `send2trash` usa invece il cestino locale al container (`~/.local/share/Trash`) — che va perso quando il container viene rimosso.
- **Podman rootless** mappa l'uid 1000 del container su un subuid dell'host, quindi il proprietario lato host di `.Trash-1000` è quell'id mappato, non il tuo account; leggere o ripristinare da esso sull'host può richiedere `podman unshare`.

Se vuoi che i file cestinati finiscano in un posto scelto da te, invece che in una cartella cestino nascosta per volume, `move_rejects` verso una sottocartella esplicita (vedi [Scarta in cartella](VIEWER.md#scarta-in-cartella)) resta l'alternativa — altrettanto sicura nel container, con come destinazione qualunque `target_dir` tu indichi.
...
`config/` porta i valori predefiniti forniti, quindi nessun `scoring_config.json`
viene sincronizzato: il NAS funziona con quei valori a meno che tu non metta il tuo
override accanto al viewer. Se ne hai uno sulla macchina di scoring, aggiungilo
all'elenco `rsync` — è un file specifico dell'installazione, quindi controllalo
prima (contiene la password del viewer ed eventuali chiavi API in chiaro).
...
Per un NAS solo viewer in cui l'immagine deve restare piccola (senza CUDA), compila invece un'immagine snella. Nota che la protezione CI richiede che ogni sorgente `COPY` sia tracciata da git, quindi il contesto di build deve includere i file elencati — motivo per cui nessun `scoring_config.json` viene copiato: quel file è un override specifico dell'installazione, non tracciato da git, e la sua assenza significa semplicemente che il container funziona con i valori predefiniti dentro `config/`.
