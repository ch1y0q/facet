...
Diferente de Selecionar para pasta, que deixa a linha do banco de dados intacta até uma nova varredura, Excluir remove a linha da foto imediatamente — nenhum `--cleanup-missing-photos` é necessário para que ela desapareça da galeria.
...
**Resultados parciais.** A resposta é por caminho, nunca tudo ou nada: `dry_run`, `would_trash` (prévia da simulação), `deleted`, `not_found` (nunca no banco de dados), `not_visible` (no banco de dados, mas oculto para este usuário), `refused_bracket_lead`, `sequence_siblings` (fotogramas trazidos pela flag de irmãos), `trashed` (uma contagem) e `errors` (um erro do sistema por caminho que falhou). Uma exclusão em lote que inclui um líder de bracket recusado ainda assim exclui tudo o mais e reporta a recusa, em vez de falhar a requisição inteira. `dry_run` tem padrão `true`.
