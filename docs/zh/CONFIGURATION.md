...
缩略图都在 8-bit SDR sRGB 空间工作，因此 PQ 图像会先按相机的原生位深解码、
做 BT.2020 到 sRGB 的原色转换、在线性光下做色调映射，最后套用 sRGB OETF——
只有在最后这一步才收窄到 8 bit。本段控制这一色调映射。它**仅**作用于 NCLX 色彩配置声明为 PQ 传递函数（特征值 16）的
...
PQ 编码的是绝对亮度：无论画面实际达到多亮，其码值范围都要铺满 0 到 10,000
尼特的整条 ST 2084 标度。在 8 bit 下，256 个码值中只有约 130 个落在漫反射白
以下，而整整四分之一的范围覆盖的是 1000-10,000 尼特——以 1000 尼特制作母版的
相机根本到不了那里。因此，在 EOTF 展开这些码值之前就解码成 8 bit，会让平滑
渐变出现色阶断层。

现在解码会向 HEIF 解码器索取文件自身的位深（当前相机为 10 bit），EOTF 查找表
也按该位深建表：收窄到 8 bit 发生在色调曲线之后，而不是之前。在 1024 级 PQ
灰阶上实测：8 bit 通路只得到 170 个不同的输出级、中间有 86 个空档，原生通路
得到全部 256 个、没有空档。

这对照片没有任何可测量的影响——传感器噪声本就把 8 bit 的台阶抖散了，真实的
佳能 PQ 照片两种通路都填满 256 个亮度分箱中的 248 个，其裁剪百分比、
`shadow_clipped` / `highlight_clipped` 标志与 `exposure_score` 均保持不变。
收益体现在平滑的合成内容上：晴朗天空、影棚背景、大光圈虚化。内存峰值并不会
因此上升：解码器的 uint16 缓冲区确实更宽，但走这条通路意味着 Pillow 根本不再
解码该图像，因此一张 1200 万像素的 PQ 图像峰值为 166.4 MiB，而 8 bit 通路为
212.3 MiB。SDR HEIF 永远不会走这条通路。若解码器无法提供原生位深，会静默回退
到 8 bit 通路，而 `enabled: false` 则整段跳过。
