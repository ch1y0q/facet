...
<p align="center">
  <img src="docs/screenshots/walkthrough.gif" alt="Facet in azione — galleria, punteggio per foto, selezione, capsule, cronologia, mappa e statistiche" width="100%">
</p>

## Come funziona

1. **Scansione** — Indica a Facet una cartella di foto. Ogni immagine viene analizzata per qualità, composizione e volti. Supporta JPG, HEIF/HEIC/HIF (gli scatti `.HIF` Canon HDR PQ vengono decodificati con la funzione di trasferimento SMPTE ST 2084 PQ e automaticamente sottoposti a tone mapping in sRGB SDR), 10 formati RAW (CR2, CR3, NEF, ARW, RAF, RW2, DNG, ORF, SRW, PEF) e immagini PNG, GIF, WebP, BMP, TIFF e AVIF (AVIF richiede un Pillow compilato con il supporto AVIF, nativo a partire da 11.3).
...
- **Contesti di punteggio** — controlla *quale* categoria valuta una foto, indipendentemente dai cursori dei pesi, che regolano solo la categoria già scelta: riordina la priorità globale delle categorie, applica un contesto con nome (Azione/Palco, Sessione ritratto, Fauna selvatica, …) per album, oppure imposta una sovrascrittura di categoria per singola foto che sopravvive a ogni ricalcolo.
